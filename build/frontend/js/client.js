  ...
  if (users.length <= 0) {
    ...
    navigator.webkitGetUserMedia({ video: true, audio: true }, function (myStream) {
      ...
      users.forEach(function (user, i) {
        ...
        createRemoteVideo(videoArea, null, function (err, remoteVideo) {
          if (err) {
            console.log(err);
          }
          ...
          if (remoteVideo) {
            ...
            initRtcPeerClient(iceServers, userID, remoteVideo, function (err, client) {
...
function handleLeaveRoom() {
  ...
  stream.getTracks().forEach(function (track, i) {
    track.stop();
  });

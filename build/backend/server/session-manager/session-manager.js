...
var events = require('events'), util = require('util'), RedisManager = require('./redis-manager');
...
var SessionManager = /** @class */ (function () {
    function SessionManager(config, callback) {
        if (config)
            this.conf = config;
        ...
        this.redisClient.on("error", function (err) {
            ...
            if (callback)
                callback("Redis error encountered :" + err);
        });
        this.redisClient.on("end", function (err) {
            logger.warn("Redis connection closed");
            if (callback)
                callback('ERR-REDIS: ' + 'failed to connect to Redis server(s). ');
        });
